...
function updatePlan(
  plan: BudgetPlan,
  timestamp: Timestamp,
  updates: Partial<Pick<BudgetPlan, "balanceSnapshots" | "financialEvents" | "plannedRecords">>,
): BudgetPlan {
  return {
    ...plan,
    updatedAt: timestamp,
    ...
    plannedRecords: {
      categories: [
        ...(updates.plannedRecords?.categories ?? plan.plannedRecords.categories),
      ],
      incomeTemplates: [
        ...(updates.plannedRecords?.incomeTemplates ??
          plan.plannedRecords.incomeTemplates),
      ],
      commitmentTemplates: [
        ...(updates.plannedRecords?.commitmentTemplates ??
          plan.plannedRecords.commitmentTemplates),
      ],
      savingsGoals: [
        ...(updates.plannedRecords?.savingsGoals ?? plan.plannedRecords.savingsGoals),
      ],
      flexibleCategoryGuidance: [
        ...(updates.plannedRecords?.flexibleCategoryGuidance ??
